...
def export_pipeline_telemetry(model_instance, export_path="models/metrics.json"):
    ...
    try:
        ...
        if y_pred is None:
            raise ValueError("Model predictions (y_pred) are missing. Ensure evaluate() was executed.")
        ...
        print(f"[EXPORT] Successfully generated neural network telemetry metrics at: {export_path}")
        ...
        print(f"[WARN] Telemetry extraction failed ({str(e)}). Exported baseline telemetry to: {export_path}")
...
def main():
    ...
    try:
        ...
        model.extract_features(max_features=40000)
