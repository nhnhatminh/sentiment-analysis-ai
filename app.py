"""
ReviewClassifyAI - Hệ Thống Phân Loại Cảm Xúc
UI/Dashboard Development & Data Visualization
"""
...
DATA_CLEAN      = ROOT / "data"  / "clean.csv"
...
# ──────────────────────────────────────────────────────────
# LUCIDE ICONS (Tích hợp native SVG)
# ──────────────────────────────────────────────────────────
...
def load_model(mp: pathlib.Path, vp: pathlib.Path):
    if not mp.exists() or not vp.exists(): return None, None
    try:
        with open(mp, "rb") as f:  model      = pickle.load(f)
        with open(vp, "rb") as f:  vectorizer = pickle.load(f)
...
def predict_single(text: str, model, vectorizer) -> tuple:
    ...
    # Tình huống Fallback nếu import lỗi (bảo vệ Dashboard không sập)
...
# ──────────────────────────────────────────────────────────
# BỘ CÔNG CỤ VẼ BIỂU ĐỒ (PLOTLY FIXES)
# ──────────────────────────────────────────────────────────
...
def chart_wordcloud(df: pd.DataFrame):
    ...
    # Tối ưu FIX lỗi TrueType Font mạnh mẽ cho Windows
    font_path = None
    if os.name == "nt":
    ...
    except Exception: # Bắt sạch mọi lỗi kể cả ValueError
        try:
            # Lần fallback cuối cùng
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # LOGIC NÚT ĐIỀU HƯỚNG ĐÃ FIX TỐI ƯU CỰC KỲ NỔI BẬT
